...
def sidebar():
    ...
    uploaded_file = st.sidebar.file_uploader("Selecione o arquivo Excel",
                                             type=["xlsx", "xls"],
                                             key="file_uploader")
...
def main():
    st.set_page_config(page_title="Sistema de Cobranças",
                       page_icon="💰",
                       layout="wide",
    ...
    if st.session_state.filtered_df is not None:
        ...
        tab1, tab2 = st.tabs(
            ["Distribuição por Dias", "Distribuição por Valor"])

        with tab1:
            st.subheader("Dias em Atraso")
            st.bar_chart(st.session_state.filtered_df['atraso'].value_counts().
                         sort_index(),
                         height=400)

        with tab2:
            st.subheader("Valores Devidos")
            st.area_chart(st.session_state.filtered_df.groupby('atraso')
                          ['valortotal'].sum(),
                          height=400)

        st.header("Top 10 Maiores Devedores")
        top_devedores = st.session_state.filtered_df.nlargest(10, 'valortotal')
        st.dataframe(top_devedores[['nome', 'atraso', 'valortotal']],
                     height=400,
                     use_container_width=True)
    else:
        st.warning(
            "Nenhum dado disponível. Carregue um arquivo Excel na sidebar.")
