...
def exibir_acoes_cobranca_tab():
    ...
    with col2:
        sort_options = {
            "Data da Próxima Cobrança": ("data_cobranca", True), "Fase da Cobrança": ("fase_cobranca", True),
            "Nome (A-Z)": ("nome", True), "Valor da Dívida": ("valortotal", False), "Dias em Atraso": ("atraso", False),
        }
    ...
    df_pagina = cached_get_paginated_data(st.session_state.page_num_acoes, PAGE_SIZE, sort_column, ascending, filtro_nome)
    
    st.markdown(f"--- \nExibindo **{len(df_pagina)}** de **{total_items}** devedor(es).")

    col_pag_1, col_pag_2, col_pag_3 = st.columns([1, 2, 1])
    if col_pag_1.button("⬅️ Anterior", use_container_width=True, disabled=(st.session_state.page_num_acoes == 0)):
        st.session_state.page_num_acoes -= 1
        st.rerun()
    col_pag_2.write(f"<div style='text-align: center;'>Página {st.session_state.page_num_acoes + 1} de {total_pages}</div>", unsafe_allow_html=True)
    if col_pag_3.button("Próxima ➡️", use_container_width=True, disabled=(st.session_state.page_num_acoes >= total_pages - 1)):
        st.session_state.page_num_acoes += 1
        st.rerun()
...
def exibir_calendario_cobrancas_tab():
    ...
    col1, col2 = st.columns(2)
    year = col1.selectbox("Ano", range(date.today().year - 2, date.today().year + 3), index=2, key="cal_year")
    month = col2.selectbox("Mês", range(1, 13), format_func=lambda m: calendar.month_name[m], index=date.today().month - 1, key="cal_month")
    ...
    for day, count in events_by_day.items():
        ...
        month_html = month_html.replace(f'>{day}</td>', f'><div class="day-cell">{day}{event_html}</div></td>', 1)

    if year == date.today().year and month == date.today().month:
        day_str = str(date.today().day)
        month_html = month_html.replace(f'<div class="day-cell">{day_str}', f'<div class="day-cell today">{day_str}')

    st.markdown(month_html, unsafe_allow_html=True)
    st.markdown("""<style> table { width: 100%; border-collapse: collapse; } th { background-color: #f4f4f4; padding: 8px; text-align: center; } td { border: 1px solid #ccc; height: 90px; vertical-align: top; padding: 5px; text-align: right; position: relative; } td.noday { background-color: #f9f9f9; } .day-cell { font-size: 16px; } .event-count { background-color: #0d6efd; color: white; font-size: 12px; padding: 2px 6px; border-radius: 12px; display: inline-block; position: absolute; top: 4px; left: 4px; } .today { background-color: #e8f4ff; border: 2px solid #0d6efd; border-radius: 6px; padding: 2px 6px; display: inline-block; } </style>""", unsafe_allow_html=True)

    # --- Lista de Devedores por Data ---
    st.markdown("---")
    st.subheader("Ver cobranças para uma data específica")

    # Este é o filtro que causava o problema.
    ...
    if selected_date_input != st.session_state.selected_date:
        ...
        st.session_state.page_num_cal = 0
        st.rerun() # Apenas o rerun é necessário. O Streamlit manterá a aba.
    ...
    if total_items == 0:
        ...
        col_pag_1, col_pag_2, col_pag_3 = st.columns([1, 2, 1])
        if col_pag_1.button("⬅️ Anterior", key="cal_prev", use_container_width=True, disabled=(st.session_state.page_num_cal == 0)):
            st.session_state.page_num_cal -= 1
            st.rerun()
        col_pag_2.write(f"<div style='text-align: center;'>Página {st.session_state.page_num_cal + 1} de {total_pages}</div>", unsafe_allow_html=True)
        if col_pag_3.button("Próxima ➡️", key="cal_next", use_container_width=True, disabled=(st.session_state.page_num_cal >= total_pages - 1)):
            st.session_state.page_num_cal += 1
            st.rerun()
...
def main():
    ...
    # A renderização das abas agora é direta e limpa.
    # O Streamlit gerencia qual aba está ativa.
