...
from devedores_service import (load_devedores_from_db, add_devedor_to_db,
...
def initialize_session_state():
    defaults = {
        'df': None,
        'filtered_df': None,
        'edited_df_state': None,
        'page_number': 1,
        'items_per_page': 25,
        'search_term_state': "",
        'should_reload_df': True,
...
def sidebar_content():
    ...
    with st.sidebar:
        ...
        if uploaded_file:
            ...
            success, message = import_excel_to_db(st.session_state.db_engine,
                                                  uploaded_file)
            if success:
                st.success(message)
        ...
        if st.session_state.df is not None and not st.session_state.df.empty:
            excel_data, export_message = export_devedores_to_excel(
                st.session_state.df)
            if excel_data:
                st.download_button(
    ...
    if st.session_state.df is None or st.session_state.df.empty:
        filters['original_valor_min'], filters[
            'original_valor_max'] = 0.0, 10000.0
...
def apply_filters(df, filters):
    ...
    filtered = df.copy()
    filtered['valortotal'] = pd.to_numeric(filtered['valortotal'],
                                           errors='coerce').fillna(0)
    filtered['atraso'] = pd.to_numeric(filtered['atraso'],
                                       errors='coerce').fillna(0)
    ...
    if filters['valor_range'] != (filters['original_valor_min'],
                                  filters['original_valor_max']):
        filtered = filtered[
            (filtered['valortotal'] >= filters['valor_range'][0])
            & (filtered['valortotal'] <= filters['valor_range'][1])]

    if filters['dias_range'] != (filters['original_dias_min'],
                                 filters['original_dias_max']):
        ...
        if not st.session_state.filtered_df.empty:
            ...
            if excel_data:
                st.download_button(
                    ...
                    data=excel_data,
                    file_name=
                    f"devedores_filtrados_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime=
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
...
def process_table_edits(edited_df, original_df):
    ...
    for idx in changed_rows.index:
        ...
        changes = {
            ...
            if edited_df.loc[idx, col] != original_df.loc[idx, col]
        }

        if 'status' in changes and str(
                changes['status']) == StatusDevedor.PAGO.value:
            changes['data_pagamento'] = datetime.now().date()

        success, message = update_devedor_in_db(st.session_state.db_engine,
                                                devedor_id, changes)
        if success:
            updates_processed += 1
        else:
    ...
    if updates_processed:
        st.success(
            f"{updates_processed} registro(s) atualizado(s) com sucesso!")
        st.session_state.should_reload_df = True
...
def show_lista_devedores_tab(filters):
    ...
    with st.expander("➕ Adicionar Novo Devedor"):
        with st.form("novo_devedor_form", clear_on_submit=True):
            ...
            valor = c3.number_input("Valor Devido (R$)*",
                                    min_value=0.01,
                                    format="%.2f")
            ...
            if st.form_submit_button("Adicionar Devedor", type="primary"):
                if not nome or valor <= 0:
                    ...
                    success, message = add_devedor_to_db(
                        st.session_state.db_engine, nome, valor, atraso,
                        telefone, pessoa_id)
                    if success:
                        st.success(message)
                        st.session_state.should_reload_df = True
    ...
    if st.session_state.should_reload_df:
        ...
        if st.session_state.df is not None:
            st.session_state.df = st.session_state.df.sort_values(
                by=['atraso', 'valortotal'], ascending=[False, False])
    ...
    if st.session_state.confirming_delete:
        st.error(
            f"### Deseja realmente excluir {len(st.session_state.ids_to_delete)} registro(s)?"
        )
        st.warning("Esta ação não pode ser desfeita.")

        col1, col2 = st.columns(2)
        if col1.button("Sim, Excluir Agora",
                       type="primary",
                       use_container_width=True):
            sucessos, falhas = 0, 0
            for devedor_id in st.session_state.ids_to_delete:
                success, message = remover_devedor_from_db(
    ...
    st.info(
        f"📊 **Total de registros:** {total_registros_filtrados} de {total_registros_original} (filtrados)"
    )
    ...
    if st.session_state.items_per_page > total_registros_filtrados and total_registros_filtrados > 0:
        st.session_state.items_per_page = min(
            items_per_page_options,
            key=lambda x: abs(x - total_registros_filtrados))
    ...
    total_pages = max(1,
                      int(np.ceil(total_registros_filtrados / items_per_page)))
    st.session_state.page_number = min(st.session_state.page_number,
                                       total_pages)
    ...
    with page_col1:
        ...
        if nav_cols[0].button("⏪ Anterior",
                              disabled=(st.session_state.page_number == 1),
                              key="prev_page_main",
                              use_container_width=True):
            st.session_state.page_number -= 1
            st.rerun()
        if nav_cols[1].button("Próxima ⏩",
        ...
        nav_cols[2].markdown(
            f"**Página {st.session_state.page_number} de {total_pages}** (Itens {start_item}-{end_item})"
        )
    ...
    if not display_df.empty:
        ...
        display_df['status'] = pd.Categorical(display_df['status'],
        ...
        column_config = {
            ...
            st.column_config.CheckboxColumn(
                "Excluir?",
                default=False,
                help="Marque para remover o devedor."),
            "pessoa":
            st.column_config.TextColumn("ID Pessoa", disabled=True),
            "nome":
            st.column_config.TextColumn("Nome", disabled=True),
            "valortotal":
            st.column_config.NumberColumn("Valor Total",
                                          format="R$ %.2f",
                                          disabled=True),
            "atraso":
            st.column_config.NumberColumn("Dias Atraso",
                                          format="%d dias",
                                          disabled=True),
            "status":
            st.column_config.SelectboxColumn("Status Atual",
                                             options=status_options,
                                             required=True),
            "telefone":
            st.column_config.TextColumn("Telefone", disabled=True),
            "data_pagamento":
            st.column_config.DateColumn("Data Pagamento",
                                        format="DD/MM/YYYY",
                                        disabled=True)
        }
        ...
        edited_df = st.data_editor(display_df,
                                   ...
                                   hide_index=True,
                                   disabled=display_df.columns.drop(
                                       ['status', 'Excluir']),
                                   key="devedores_editor")

        action_col1, action_col2 = st.columns(2)

        if action_col1.button("💾 Salvar Alterações de Status",
                              use_container_width=True):
            if 'page_df_before_edit' not in st.session_state or not st.session_state.page_df_before_edit.equals(
                    display_df):
        ...
        if not linhas_para_excluir.empty:
            if action_col2.button(
                    f"🗑️ Excluir {len(linhas_para_excluir)} Registro(s)",
                    type="primary",
                    use_container_width=True):
                st.session_state.confirming_delete = True
                st.session_state.ids_to_delete = linhas_para_excluir[
                    'id'].tolist()
                st.rerun()
    else:
